...
const WithdrawModal = ({onClose}) => {
  ...
  const  handleSubmission = async(e)=>{
    ...
    try {
      // check for one field 
      if(account.length===0){
        return toast.error("Please add at least one fied");
      }
      ...
      <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-auto flex flex-col'>
        ...
        <form onSubmit={handleSubmission} className='flex flex-col items-start gap-4 p-4 overflow-y-scroll'>
          <div className='grid grid-cols-[2fr_3fr_1fr] items-center gap-2' >
            Amount <input type="number" onChange={(e)=>setAmount(e.target.value)} value={amount} className='w-full px-2 py-1.5 text-sm border border-gray-300 rounded outline-indigo-400' required />
         ...
         {account.map((field,index)=>(
                 ...
                 {/* submit button */}
                <button className='bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 mt-4 rounded-md'>
                    Apply for Withdrawl
                </button>
