...
export const getDashboard = async(req,res)=>{
    try{
        ...
        const totalRevenue = transaction.reduce((total,transaction)=>total + transaction.amount,0)
...
// controller for gettinng all listings
 ...
 // controller for getting all unverified listings with credentials submistted
 export const getAllUnverifiedListings = async (req,res)=>{
    try {
        const listings = await prisma.listing.findMany({
            where  :{isCredentialSubmitted:true,
 ...
 export const getCredential = async (req,res)=>{
    try {
        ...
        if(!credential){
            return res.status(404).json({message:"Credenial not found"});
 ...
 // mark credential as verified
  ...
  export const getAllTransactions = async (req, res) => {
    try {
        ...
        // Get Customer Details for each transaction and add it to the tran
        // object
...
export const getAllWithdrawnRequests = async(req,res)=>{
    try {
...
export const markWithdrawalAsPaid= async(req,res)=>{
    try {
        ...
        return res.json({message:"Withdrawl marked as paid"})
    } catch (error) {
        console.log(error);
        res.status(400).json({message: error.code || error.message});
    }
